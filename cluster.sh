...
configure_api_key_secret() {
    ...
    log "Managing llmops-env secret..."
    kubectl get secret llmops-env -n "$model_serving_namespace" >/dev/null 2>&1 && \
        kubectl delete secret llmops-env -n "$model_serving_namespace"

    kubectl create secret generic llmops-env --from-env-file="$env_file_path" -n "$model_serving_namespace" || \
