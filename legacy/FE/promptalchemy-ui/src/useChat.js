...
const useChat = () => {
  ...
  const sendMessage = useCallback(
    async (inputMessage, settings) => {
      setIsLoading(true);
      setIsTyping(true);

      try {
        // const response = await fetch('http://localhost:30000/api/llmops_conversation/conversation', {
        const response = await fetch(
          "http://35.240.206.150.nip.io/api/llmops_conversation/conversation",
          {
            method: "POST",
