...
def setup_prometheus() -> metrics.Meter:
    ...
    try:
        ...
        resource = Resource(attributes={SERVICE_NAME: 'llmops-service'})
        ...
        meter = metrics.get_meter('myllmops', '0.3.6')
...
def setup_jaeger() -> TracerProvider:
    ...
    try:
        ...
        set_tracer_provider(
            TracerProvider(resource=Resource.create({SERVICE_NAME: 'llmops-service-trace-manual'}))
        )

        # Get Jaeger tracer
        tracer = get_tracer_provider().get_tracer('myllmops-trace', '0.1.2')
...
counter = meter.create_counter(
    name='llmops_request_counter',
    description='Number of llmops requests',
)
histogram = meter.create_histogram(
    name='llmops_response_histogram',
    description='llmops response histogram',
    unit='seconds',
)
